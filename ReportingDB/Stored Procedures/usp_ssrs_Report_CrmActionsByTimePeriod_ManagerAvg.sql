...
--					2 - по неделям
--					3 - по месяцам
--				Процедура возвращает среднее кол-во выполненных действий в расчете на 1 менеджера 
--				(за исключением тестовых) отдела (DepartmentID) Sales Force, сгруппированных по 
--				выбранному временному периоду. 
--				Для целей отчета отдельно выделяем тип действия - "Встреча"
-- ======================================================================================================

CREATE PROCEDURE [dbo].[usp_ssrs_Report_CrmActionsByTimePeriod_ManagerAvg] 
...
BEGIN
	...
	SELECT 
	   YearNum, TimePeriod, TimeOrder, CrmActionType
	 , AVG(1. * CrmActionCount) CrmActionCountPerManager
	 , SUM(AVG(1. * CrmActionCount)) OVER(PARTITION BY YearNum, TimePeriod, TimeOrder) AS CrmActionTotal
	FROM C
	GROUP BY YearNum, TimePeriod, TimeOrder, CrmActionType
