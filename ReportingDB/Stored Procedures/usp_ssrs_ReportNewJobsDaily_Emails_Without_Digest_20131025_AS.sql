...
CREATE PROCEDURE [dbo].[usp_ssrs_ReportNewJobsDaily_Emails_Without_Digest_20131025_AS]
...
CREATE TABLE #T (Date DATETIME, SubscribeCompetitorID INT, TotalCount INT, NewVacancyCount INT)

CREATE INDEX IX_SubscribeCompetitorID ON #T (SubscribeCompetitorID)

INSERT INTO #T (Date, SubscribeCompetitorID, TotalCount, NewVacancyCount)
SELECT Date, SubscribeCompetitorID, TotalCount, NewVacancyCount
FROM Statistic.dbo.VacancySubscription_History VSH WITH (NOLOCK)
WHERE Date BETWEEN @StartDate AND @EndDate AND DATEPART(WEEKDAY,Date) <> 1
...
FROM SRV16.RabotaUA2.dbo.SubscribeCompetitor SC WITH (NOLOCK)
	JOIN #T VSH ON VSH.SubscribeCompetitorID = SC.Id
WHERE SC.SourceType NOT IN (2,7)
