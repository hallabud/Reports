...
CREATE PROCEDURE [dbo].[usp_ssrs_Report_Forecast_Grouped] (@StartDate DATETIME, @EndDate DATETIME)
...
SELECT 
 ...
 , D.FirstPaymentDate
 , CASE 
    WHEN EXISTS (SELECT * FROM SRV16.RabotaUA2.dbo.OrderDetail WHERE OrderID = O.ID AND UnitID = 2) 
	 THEN (SELECT MAX(Count) FROM SRV16.RabotaUA2.dbo.OrderDetail WHERE OrderID = O.ID AND UnitID = 2) * 30
...
SELECT D.Name AS Department, AM.Email, M.Name AS Manager, ISNULL(ML.Name, M.Name) AS TeamLead, ForecastGroup, ForecastGroupOrderNum, SUM(PosAccSum) AS SumPosAccSum 
FROM (
	...
	UNION ALL
	SELECT * FROM C_Deals) AS Total
 JOIN SRV16.RabotaUA2.dbo.aspnet_Membership AM WITH (NOLOCK) ON Total.Manager = AM.Email 
 JOIN SRV16.RabotaUA2.dbo.Manager M WITH (NOLOCK) ON AM.UserID = M.aspnet_UserUIN AND M.DepartmentID IN (1,2,3,4,10,12) AND M.IsForTesting = 0
 JOIN SRV16.RabotaUA2.dbo.Departments D WITH (NOLOCK) ON M.DepartmentID = D.ID
 LEFT JOIN SRV16.RabotaUA2.dbo.Manager ML WITH (NOLOCK) ON M.TeamLead_ManagerID = ML.ID
WHERE M.IsLoyaltyGroup = 0
GROUP BY D.Name, AM.Email, M.Name, ISNULL(ML.Name, M.Name), ForecastGroup, ForecastGroupOrderNum;
