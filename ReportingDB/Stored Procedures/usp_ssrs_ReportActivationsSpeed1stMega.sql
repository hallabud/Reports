...
CREATE PROCEDURE [dbo].[usp_ssrs_ReportActivationsSpeed1stMega]
...
SELECT
 ...
 , (SELECT MIN(DateOfUpdate) FROM SRV16.RabotaUA2.dbo.Notebook_History 
     WHERE [Key] = NC.NotebookId AND Field = 'NotebookStateId' AND NewValue = 7) AS 'FirstMegaStatus'
...
SELECT
 ...
 , OrderNum
 , CASE 
    WHEN DATEDIFF(DAY,RegDate,FirstMegaStatus) >= 15 THEN '15+'
    ELSE CAST(DATEDIFF(DAY,RegDate,FirstMegaStatus) AS VARCHAR)
   END AS 'DaysBeforeMegaStatus'
FROM NC_Flags
WHERE FirstMegaStatus IS NOT NULL AND DATEDIFF(WEEK,RegDate,FirstMegaStatus) >= 0
 )
SELECT 
   TotalVacsGroup
 , CountEmpGroup
 , OrderNum 
 , DaysBeforeMegaStatus
